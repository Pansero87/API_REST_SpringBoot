...
public class OrchestraController {
     ...
     * for displaying a list of orchestras.
     *
     * @return ModelAndView object representing the orchestras list view
     */
    @GetMapping("/orchestras/view-list")
    public ModelAndView getAllMusiciansView() {
        ModelAndView mav = new ModelAndView("orchestrasList");
        try {
            logger.info("Getting all orchestras");
            List<OrchestraDTO> orchestrasList = orchestraService.getAllOrchestras();
            mav.addObject("orchestras", orchestrasList);
            logger.info("Orchestras retrieved successfully");
            return mav;
        } catch (Exception e) {
